 ...
 * 
 * A árvore binária é uma estrutura de dados onde cada nó possui no máximo dois
 * filhos,
...
public class BinaryTree {
    ...
    /**
     * Instância da classe Balance, responsável pelo balanceamento da árvore.
     */
    Balance treeBalancing;

    /**
     * Construtor da classe. Inicializa a raiz da árvore como null e 
     * a instância de Balance para balancear a árvore.
     */
    public BinaryTree() {
        root = null;
        treeBalancing = new Balance();
    }
     ...
     * Se o valor do nó a ser inserido é menor que o valor do nó atual, ele é
     * inserido à esquerda. Se é maior, ele é inserido à direita.
     * Se o valor já existe, as linhas do nó atual são atualizadas.
     ...
     * 
     * Verifica a posição correta do novo nó e insere-o na árvore.
     * Após a inserção, ajusta a altura do nó e realiza o balanceamento, se necessário.
     * 
     * @param newNode O nó a ser inserido.
     * @param current O nó atual sendo comparado.
     * @return O nó atualizado após a inserção e o balanceamento.
     */
    private Node insertNewNode(Node newNode, Node current) {
        ...
        int leftHeight = treeBalancing.getHeight(current.getLeft());
        int rightHeight = treeBalancing.getHeight(current.getRight());

        current.setHeight(1 + Math.max(leftHeight, rightHeight));

        return treeBalancing.balancingNode(current);
    }
     ...
     * 
     * Na impressão em ordem, os filhos à esquerda são processados antes do nó
     * atual,
     ...
     * 
     * Na impressão pós-ordem, os filhos à esquerda e à direita são processados
     * antes do nó atual.
     * 
     */
